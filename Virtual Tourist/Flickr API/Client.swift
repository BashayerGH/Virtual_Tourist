...
class Client {
    ...
    func searchByLocation(latitude: Double, longitude: Double, totalPages: Int?, completion: @escaping (_ result: Parser?, _ error: Error?) -> Void) {
        ...
        _ = taskForGETMethod(parameters: parameters) { (data, error) in
            ...
            guard let data = data else {
                let userInfo = [NSLocalizedDescriptionKey : "Error found in retrieving data"]
                completion(nil, NSError(domain: "taskForGETMethod", code: 1, userInfo: userInfo))
                return
            }
            
            do {
                let photosParser = try JSONDecoder().decode(Parser.self, from: data)
...
extension Client {
    ...
    struct FlickrParameterValues {
        ...
        static let DisableJSONCallback = "1" // 1 is yes
        static let MediumURL = "url_n"
        static let UseSafeSearch = "1" //1 is safe content
